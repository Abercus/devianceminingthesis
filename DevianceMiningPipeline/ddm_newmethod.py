...
from deviancemining import filter_candidates_by_support, count_classes
...
class DRC:
    ...
    def create_data_aware_features(self, train_log, test_log, ignored):
        ...
        for key in train_df.columns:
            ...
            for pos_act, _, __ in train_negative_samples:
                for key2, val in pos_act.items():
                    collected_features.add(key2)
            ...
            positive_samples = [(sample[2], sample[0]) for sample in train_positive_samples]
            negative_samples = [(sample[2], sample[0]) for sample in train_negative_samples]
            ...
            #test_neg_smpl = [(sample[2], sample[0]) for sample in test_positive_samples if sample[1] == 0]

            pos_test_samples = self.create_sample(test_pos_smpl, features, 1)
            #pos_neg_samples = self.create_sample(test_neg_smpl, features, 0)
            test_features_data = pos_test_samples #+ pos_neg_samples
            ...
            if len(literal_features) > 0:
                for selection in literal_features:
                    ...
                    le.fit(list(train_df[selection]) + list(test_df[selection]))
                    classes = le.classes_
                    ...
                    ohe.fit(np.concatenate((train_df[selection].values.reshape(-1, 1), test_df[selection].values.reshape(-1, 1)), axis=0))
            ...
            for i, trace in enumerate(outp_train):
                ...
                freq = trace[0]
                if freq == 0:
                    # vacuous case, remain 0
                    new_train_feature.append(0)
                elif freq == -1:
                    # Violation case, remain -1, as data condition narrows
                    new_train_feature.append(-1)
                else:
                    # Fulfillment case, > 0, lets see if it will be unfulfilled now.
                    # Get all previous data
                    samples = train_df[train_df.id == trace_id]
                    samples.pop("id")
                    dv = samples.values
                    outs = data_dt.predict(dv)
                    if sum(outs) == freq:
                        new_train_feature.append(freq)
                    else:
