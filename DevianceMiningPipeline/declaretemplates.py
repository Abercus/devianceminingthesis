"""
Contains all the "kernels" for declare templates
Templates take a trace and event set, and test if the template is satisfied in the trace with given set.
...
def template_alternate_precedence(trace, event_set):
                ...
                while True:
                    ...
                    if next_event_1_pos:
                        ...
                        if event_1_pos < pos2:
                            ...
                            return 0, False  # since there is no smaller remaining event
    ...
    return 1, True  # todo: vacuity condition!!
...
def template_alternate_succession(trace, event_set):
    ...
    if event_1 in trace and event_2 in trace:
        ...
        if len(event_1_positions) != len(event_2_positions):
            return 0, False  # impossible if not same length
        ...
        while current_ind < len(event_1_positions):
            ...
            if next_pos <= pos:
                return 0, False  # next one is smaller than current
    ...
    return 1, True  # vacuity condition
...
def template_chain_precedence(trace, event_set):  # exactly 2 event
    ...
    if event_2 in trace:
        if event_1 in trace:
            ...
            if len(event_1_positions) < len(event_2_positions):
                return 0, False  # impossible to fulfill
            ...
            for i, pos2 in enumerate(event_2_positions):
                ...
                while True:
                    if event_1_ind >= len(event_1_positions):
                        return 0, False  # not enough response

                    if pos2 < event_1_positions[event_1_ind]:
                        return 0, False  # passed, no event before pos2
            ...
            return 0, False  # no response for event1

    return 1, True  # todo, vacuity
                ...
                while True:
                    ...
                    if pos1 < event_2_positions[event_2_ind]:
                        if pos1 + 1 != event_2_positions[event_2_ind]:
                            return 0, False  # next one is not straight after
                        else:
                            event_2_ind += 1
        ...
        while True:
            ...
            if current_e1 > current_e2:
                ...
                if current_e1 + 1 == current_e2:
                    return 0, False  # found a place, where they are together
                e1_ind += 1

    # How to do vacuity here? 1 by default most likely
    return 1, True  # TODO, this condition?
...
def template_not_coexistence(trace, event_set):
    ...
    if event_1 in trace and event_2 in trace:
        return 0, False  # if both in trace, then they exist together.
    elif event_1 in trace or event_2 in trace:
        return 1, False
    ...
    if event_1 in trace:
        if event_2 in trace:
            ...
            if first_event_1 < last_event_2:
                return 0, False  # in this case there is an event 2, which appears after first event
            else:
                return 1, False
...
def template_responded_existence(trace, event_set):
    ...
    return 1, True  # 0, if vacuity condition
